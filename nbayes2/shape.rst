...
:attr:`ndim` は，配列の次元数を表す属性で，ベクトルでは 1 に，行列では 2 になります．
...
それには，定数 :const:`np.newaxis` を使います [#]_ [#]_ ．
...
20行目では，第0次元 [#]_ は元のベクトルをコピーし，第1次元には大きさ 1 の新たな次元を追加しています．
...
この記法は， :attr:`shape` に ``(1, -1)`` や ``(-1, 1)`` を指定すると，それぞれ2次元の横ベクトルや縦ベクトルを簡便に作ることができます．
